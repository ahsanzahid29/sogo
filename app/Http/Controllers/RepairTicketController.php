...
class RepairTicketController extends Controller
{
    ...
    public function index()
    {
        ...
        if (Auth::user()->role_id == 4) {
            $data["allRepairTickets"] = RepairTicket::select(
                "repair_tickets.id as rp_id",
                "repair_tickets.ticket_number as rp_ticket",
                "repair_tickets.serial_number as rp_sn",
                ...
                ->join(
                    "inverters",
                    "repair_tickets.inverter_id",
                    "=",
                    "inverters.id"
                )
                ->where(
                    "repair_tickets.service_center_id",
                    "=",
                    Auth::user()->id
                )
                ->limit(3)
    ...
    public function serachSn(Request $request)
    {
        ...
        if (!$serialNoExist) {
            ...
            $inverters = Inverter::where(
                "id",
                $serialNoExist->inverter_id
            )->first();
            ...
            $data["history"] = RepairTicket::select(
                "repair_tickets.id as repairid",
                "repair_tickets.repair_request_date as repair_date",
                "repair_tickets.status as status",
                "inverters.modal_number as inverter_modal",
                "users.name as service_center_name",
                "repair_tickets.fault_detail as faultdetail",
                ...
                ->join(
                    "repair_spare_parts",
                    "repair_spare_parts.repair_id",
                    "=",
                    "repair_tickets.id"
                )
                ->join(
                    "spare_parts",
                    "repair_spare_parts.sparepart_id",
                    "=",
                    "spare_parts.id"
                )
                ->groupBy(
                    "repair_tickets.id",
                    "repair_tickets.repair_request_date",
                    "repair_tickets.status",
                    "inverters.modal_number",
                    "users.name",
                    "repair_tickets.fault_detail"
                )
                ->where("repair_tickets.serial_number", $value)
                ->get();
        }
        $data["sparePartsForSc"] = SparePartInvoiceItem::select(
            DB::raw("SUM(spare_part_invoice_items.quantity) as total_quantity"),
            ...
            ->where(
                "spare_part_invoice_items.service_center_id",
                "=",
                Auth::user()->id
            )
            ->get();
    ...
    public function partDetailForSc(Request $request)
    {
        ...
        $detail = ScSparePartQty::select(
            DB::raw("SUM(sc_spare_part_qties.quantity) as total_quantity"),
            "spare_parts.id as partid"
        )
            ->join(
                "spare_parts",
                "sc_spare_part_qties.sparepart_id",
                "=",
                "spare_parts.id"
            )
            ...
            ->where([
                ...
                [
                    "sc_spare_part_qties.service_center_id",
                    "=",
                    Auth::user()->id,
                ],
            ])
        ...
        if ($request->all()) {
            ...
            try {
                ...
                if ($request->file("fault_video")) {
                    ...
                    $sanitizedVideoName = str_replace(
                        " ",
                        "_",
                    ...
                    $faultVideo =
                        $sanitizedVideoName . "_" . time() . "." . $extension;
                    $video->move(
                        public_path("files/repairvideos"),
                        $faultVideo
                    );
                }
                ...
                if ($request->sparepart) {
                    ...
                    for ($j = 0; $j < count($request->sparepart); $j++) {
                        ...
                        $newQty =
                            $checkexistingSp->quantity -
                            $request->needed_stock[$j];
                ...
                return redirect("/all-repairtickets")->with(
                    "status",
                    "Repair ticket generated successfully"
                );
            } catch (\Exception $e) {
                DB::rollback();
    ...
    public function show($id)
    {
        ...
        $detailSc = User::where(
            "id",
            $repairTicketDetail->service_center_id
        )->first();
        ...
        if (!$repairTicketDetail) {
            ...
            if (
                $repairTicketDetail->service_center_id == Auth::user()->id ||
                Auth::user()->role_id == 1 ||
                Auth::user()->role_id == 2
            ) {
                $history = RepairTicket::select(
                    "repair_tickets.id as repairid",
                    "repair_tickets.repair_request_date as repair_date",
                    "repair_tickets.status as status",
                    "inverters.modal_number as inverter_modal",
                    ...
                    ->groupBy("repair_tickets.id",
                        ...
                        "repair_tickets.fault_detail")
                    ->where(
                        "repair_tickets.serial_number",
                        $repairTicketDetail->serial_number
                    )
                    ->get();
                $neededSpareParts = RepairSparePart::select(
                    "repair_spare_parts.stock_needed as need_qty",
                    "repair_spare_parts.current_stock as current_qty",
                    "spare_parts.name as sp_name",
                    "spare_parts.factory_code as fcode"
                )
                    ->join(
                        "spare_parts",
                        "repair_spare_parts.sparepart_id",
                ...
                return view(
                    "repairticket.view-repair-ticket",
                    compact(
                        "repairTicketDetail",
                        "history",
                        "neededSpareParts",
    ...
    public function markAsComplete($id)
    {
        if (Auth::user()->role_id == 1) {
            ...
            return redirect("/all-repairtickets")->with(
                "status",
                "Repair Ticket is completed"
            );
        } else {
            return redirect("/dashboard");
        }
    }
    public function editRepairTicket($id)
    {
        $repairTicketDetail = RepairTicket::find($id);
        if ($repairTicketDetail) {
            if (
                ...
                $neededSpareParts = RepairSparePart::select(
                    ...
                    ->get();
                $userRole = Auth::user()->role_id;
                $sparePartsForSc = SparePartInvoiceItem::select(
                    DB::raw(
                        "SUM(spare_part_invoice_items.quantity) as total_quantity"
                    ),
                    "spare_parts.name as partname",
                    "spare_part_categories.name as factorycode",
                    "spare_parts.part_type as parttype",
                ...
                return view(
                    "repairticket.edit-repair-ticket",
                    compact(
                        "repairTicketDetail",
                        "history",
                        "neededSpareParts",
        ...
        if ($request->all()) {
            ...
            try {
                if ($request->sparepart) {
                    ...
                    return redirect()->route('edit-repair-ticket', ['id' => $request->recordid])->with(
                        'status',
